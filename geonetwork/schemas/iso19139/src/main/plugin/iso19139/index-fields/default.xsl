...
<xsl:stylesheet xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template mode="index" match="//gmd:resourceConstraints//gmd:useLimitation/gmd:MD_ClassificationCode">
		<Field name="license" string="{string(.)}" store="true" index="true"/>
	</xsl:template>
	
	<xsl:template mode="index" match="//gmd:identificationInfo/gmd:MD_DataIdentification/gmd:citation/gmd:CI_Citation/gmd:otherCitationDetails/gco:CharacterString">
	...
	<xsl:template mode="index" match="//gmd:extent/gmd:EX_Extent/gmd:temporalElement/gmd:EX_TemporalExtent/gmd:extent/gml:TimePeriod/gml:beginPosition">
		<Field name="startDate" string="{string(.)}" store="true" index="true"/>
	</xsl:template>
  
	<xsl:template mode="index" match="//gmd:extent/gmd:EX_Extent/gmd:temporalElement/gmd:EX_TemporalExtent/gmd:extent/gml:TimePeriod/gml:endPosition">
		<Field name="endDate" string="{string(.)}" store="true" index="true"/>
	</xsl:template>
  ...
  <xsl:template match="*" mode="metadata">
	...
	<xsl:for-each select="*//gmd:resourceConstraints//gmd:useLimitation/gmd:MD_ClassificationCode">
		<Field name="license" string="{string(.)}" store="true" index="true"/>
	</xsl:for-each>
	
	<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->

	<xsl:for-each select="*//gmd:extent/gmd:EX_Extent/gmd:temporalElement/gmd:EX_TemporalExtent/gmd:extent/gml:TimePeriod/gml:beginPosition">
		<Field name="startDate" string="{string(.)}" store="true" index="true"/>
	</xsl:for-each>
	
	<xsl:for-each select="*//gmd:aggregationInfo/gmd:MD_AggregateInformation[gmd:initiativeType/gmd:DS_InitiativeTypeCode/@codeListValue='project']">
		<xsl:variable name="progName" select="gmd:aggregateDataSetName/gmd:CI_Citation/gmd:title/gco:CharacterString" />
		<xsl:variable name="progSummary" select="gmd:aggregateDataSetName/gmd:CI_Citation/gmd:otherCitationDetails/gco:CharacterString" />
		<xsl:variable name="progCoordinator" select="gmd:aggregateDataSetName/gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[gmd:role/gmd:CI_RoleCode/@codeListValue='principalInvestigator']/gmd:organisationName/gco:CharacterString" />
		<xsl:variable name="progFunder" select="gmd:aggregateDataSetName/gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[gmd:role/gmd:CI_RoleCode/@codeListValue='principalFunder']/gmd:organisationName/gco:CharacterString" />
		<xsl:variable name="progContributor">
			<xsl:for-each select="gmd:aggregateDataSetName/gmd:CI_Citation/gmd:citedResponsibleParty/gmd:CI_ResponsibleParty[gmd:role/gmd:CI_RoleCode/@codeListValue='principalContributor']">
				<xsl:choose>
					<xsl:when test="position() = 1">
						<xsl:value-of select="concat($apos,gmd:organisationName/gco:CharacterString,$apos)" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat(', ',$apos,gmd:organisationName/gco:CharacterString,$apos)" />
					</xsl:otherwise>
		...
		<Field name="programmeFull" string="{concat('{',$apos,'name',$apos,': ',$apos,$progName,$apos,', ',$apos,'summary',$apos,': ',$apos,$progSummary,$apos,',',$apos,'coordinator',$apos,': ',$apos,$progCoordinator,$apos,', ',$apos,'funder',$apos,': ',$apos,$progFunder,$apos,', ',$apos,'contributor',$apos,': [',$progContributor,']}')}" store="true" index="true" />
	</xsl:for-each>
  
	<xsl:for-each select="*//gmd:extent/gmd:EX_Extent/gmd:temporalElement/gmd:EX_TemporalExtent/gmd:extent/gml:TimePeriod/gml:endPosition">
		<Field name="endDate" string="{string(.)}" store="true" index="true"/>
	</xsl:for-each>
	
	  <xsl:for-each select="*//gmd:identificationInfo//gmd:CI_Citation//gmd:otherCitationDetails/gco:CharacterString">
        <Field name="suggestedCitation" string="{string(.)}" store="true" index="true"/>
      </xsl:for-each>
	
	<!-- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -->

	  <xsl:for-each select="*//gmd:CI_ResponsibleParty[gmd:role/gmd:CI_RoleCode/@codeListValue='custodian']/gmd:organisationName/gco:CharacterString">
        <Field name="organisation" string="{string(.)}" store="true" index="true"/>
      </xsl:for-each>
    ...
    <xsl:for-each select="gmd:identificationInfo//gmd:MD_DataIdentification|
	  ...
	  <xsl:for-each select="//gmd:MD_Keywords[gmd:thesaurusName/gmd:CI_Citation/gmd:title/gmx:Anchor='SeaDataNet Disciplines']/gmd:keyword/gmx:Anchor">
			<Field name="discipline" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
		
		<xsl:for-each select="//gmd:MD_Keywords[gmd:thesaurusName/gmd:CI_Citation/gmd:title/gmx:Anchor='SeaVoX Platform Categories']/gmd:keyword/gmx:Anchor">
			<Field name="platformType" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
		
		<xsl:for-each select="//gmd:MD_Keywords[gmd:thesaurusName/gmd:CI_Citation/gmd:title/gmx:Anchor='SeaDataNet Device Categories']/gmd:keyword/gmx:Anchor">
			<Field name="deviceType" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
		
		<xsl:for-each select="//gmd:MD_Keywords[gmd:thesaurusName/gmd:CI_Citation/gmd:title/gmx:Anchor='SeaDataNet Agreed Parameter Groups']/gmd:keyword/gmx:Anchor">
		...
		<xsl:for-each select="//gmd:MD_Keywords[gmd:thesaurusName/gmd:CI_Citation/gmd:title/gmx:Anchor='Marine Institute Calendar Concept Scheme']/gmd:keyword/gmx:Anchor">
			<Field name="kwMonth" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
		
		<xsl:for-each select="//gmd:MD_AggregateInformation[gmd:initiativeType/gmd:DS_InitiativeTypeCode[@codeListValue='project']]//gmd:title/gco:CharacterString">
			<Field name="programme" string="{string(.)}" store="true" index="true"/>
		</xsl:for-each>
